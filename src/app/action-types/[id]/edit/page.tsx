...
export default function EditActionTypePage({
  params,
}: {
  params: { id: string };
}) {
  ...
  useEffect(() => {
    const fetchActionType = async () => {
      try {
        const response = await fetch(`/api/action-types/${params.id}`);
        if (!response.ok) {
        ...
        setFormData({
          ...
          has_selector: data.has_selector === 1,
          has_assertion: data.has_assertion === 1,
        });
      } catch (err) {
        setError(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || "アクションタイプの更新に失敗しました");
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      <h1 className="text-2xl font-bold mb-8">アクションタイプの編集</h1>
